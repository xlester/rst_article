...
\begin{figure}[th]
	\begin{center}
		\subfigure{\label{fig:phase-a} \includegraphics[scale=0.40]{faseRST.png}}{Phase RST}
		\subfigure{\label{fig:phase-b} \includegraphics[scale=0.40]{faseAIA.png}}{Phase AIA}
	\end{center}
...
In Table. \ref{error_table} , we show the errors values of the estimated phase-shifts using our regularized method and the estimated using the AIA method. This errors are calculated as $|\alpha_k - \hat{ \alpha }_k }|$, where $\alpha_k$ and $\hat{\alpha}_k$ are the values of the actual and estimated phase-shifts for the $k-$frame, respectively. On the other hand, in Fig. (AGREGAR FIGURA DE FASES OBTENIDAS), we show the recovered phase. In Fig. 1(A) is shown the recovered phase using the regularized self-tuning demodulation method presented here, while in Fig. 1(B) is shown the recovered phase using the AIA method. We can see in this figure that our proposed regularized self-tuning demodulation method recovers the phase with less error than with the AIA method. The errors shown in Fig. 1.(A) and Fig. 1.(B) are calculated as the variance between the recovered phase map and the true phase map used to generate the interferograms.
\begin{table}
	\begin{center}
